...
function ForgotPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await axios.put('http://localhost:8080/set-password', null, {
                params: {
                    email: email,
                    newPassword: newPassword
                }
            });
            setMessage('Password has been set successfully. Please login to continue.');
            setError('');
        } catch (err) {
        ...
        <div className="flex justify-center items-center h-auto p-6">
            <div className=" w-full flex flex-col gap-9">
                <div className="flex justify-center items-center">
                    <h1 className="text-4xl footer-title text-slate-800">Forgot Password</h1>
                </div>
                <form onSubmit={handleSubmit} className="flex flex-col justify-center items-center gap-8">
                    ...
                    <label className="form-control w-full max-w-xs">
                        <div className="label">
                            <span className="label-text text-xl font-mono">New Password: </span>
                            <span className="label-text-alt">required</span>
                        </div>
                        <input
                            type="password"
